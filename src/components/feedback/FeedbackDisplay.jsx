...
export default function FeedbackDisplay() {
  ...
  const handlePrevious = () => {
    setCurrentIndex((prev) =>
      prev > 0 ? prev - 1 : filteredFeedbacks.length - 1
    );
  };

  const handleNext = () => {
    setCurrentIndex((prev) =>
      prev < filteredFeedbacks.length - 1 ? prev + 1 : 0
    );
  };
        ...
        <div className="flex justify-between items-center mb-4">
          <TabsList>
            ...
            >
              ...
              <span>{language === "en" ? "Testimonies" : "עדויות"}</span>
            </TabsTrigger>
            <TabsTrigger value="statistics" className="flex items-center gap-1">
              <BarChart3 className="w-4 h-4" />
              <span>{language === "en" ? "Statistics" : "סטטיסטיקות"}</span>
            </TabsTrigger>
          </TabsList>
        ...
        <TabsContent value="testimonials" className="mt-0">
          {filteredFeedbacks.length > 0 ? (
                      ...
                      <div className="flex-grow">
                        <div className="flex justify-between items-start mb-1">
                          <h3 className="font-medium">
                            {filteredFeedbacks[currentIndex].user_name}
                          </h3>
                          <div className="flex gap-1 items-center">
                            {renderStars(
                              filteredFeedbacks[currentIndex].rating
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2 mb-3">
                          ...
                          {filteredFeedbacks[currentIndex].amount_saved > 0 && (
                            <Badge variant="outline">
                              חסך ₪
                              {filteredFeedbacks[currentIndex].amount_saved}
                            </Badge>
                          )}
                        ...
                        <div className="text-sm text-gray-500 mt-3">
                          {new Date(
                            filteredFeedbacks[currentIndex].created_date
                          ).toLocaleDateString()}
                        </div>
                      </div>
                    </div>
            ...
            <div className="text-center p-6 bg-gray-50 rounded-lg">
              ...
              <p className="text-gray-500">
                {language === "en"
                  ? "There is no evidence that meets the filter criteria you selected"
